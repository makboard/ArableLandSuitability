...
with open(
    os.path.join("..", "data", "processed_files", "pkls", "X_TEST.pkl"), "rb"
) as fp:
    X = pickle.load(fp)

with open(
    os.path.join("..", "data", "processed_files", "pkls", "y_TEST.pkl"), "rb"
) as fp:
    y = pickle.load(fp)
...
network = Crop_MLP(output_size=5)
model = Crop_PL(net=network, num_classes=5)
