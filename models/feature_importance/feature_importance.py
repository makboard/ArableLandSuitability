...
from src.model_utils import CropLSTM, CropMLP, CropConvLSTM, CropTransformer, CropPL
...
pathResults = os.path.join("..", "..", "data", "results", "feature_importance")
...
with open(
    os.path.join(path_to_pkls, "X_down.pkl"),
    "rb",
) as fp:
...
with open(
    os.path.join(path_to_pkls, "X_down_lstm.pkl"),
    "rb",
) as fp:
...
with open(
    os.path.join(path_to_pkls, "y_down.pkl"),
    "rb",
) as fp:
    y = pickle.load(fp)
...
def predict_prob(
    ...
    if model_name in ["mlp", "lstm", "transformer", "conv_lstm"]:
        ...
        input_tensor = (
            X_lstm if model_name in ["lstm", "transformer", "conv_lstm"] else X_test_t
        )
        predictions = torch.cat(
            trainer.predict(loaded_model, DataLoader(input_tensor, batch_size=2048)),
            dim=0,
        )
...
def estimate_feature_importance_models(
    clf_dict, X_dict, y, feature_names_dict, num_permutations, sklearn_binary_metric
):
    """
    ...
    for model_name in tqdm(clf_dict):
        ...
        feature_importance_scores = {model_name: {} for model_name in y_probs.keys()}

        # Collect list of features
        if (
            model_name == "lstm"
            or model_name == "conv_lstm"
            or model_name == "transformer"
        ):
        ...
        for feature_dim, feature_name in tqdm(enumerate(feature_names)):
            ...
            for i in range(num_permutations):
                ...
                importance_score = (
                    original_score[model_name] - shuffled_score[model_name]
                )
        ...
        with open(os.path.join(pathResults, filename), "wb") as fs:
            pickle.dump(feature_importance_scores[model_name], fs)


def main():
    estimate_feature_importance_models(
        clf_dict,
        X_dict,
        y,
        feature_names_dict,
